...
export const Editcontact = () => {
	...
	const [inputName, setInputName ] = useState(store.contacts[params.theid].full_name);
	...
	const [inputAdress, setInputAdress ] = useState(store.contacts[params.theid].address);
			...
			<Link to="/">
				<div className="d-grid gap-2">
					<button
						...
						type="button"
						onClick={()=>(inputName!=='' && inputEmail!=='' && inputPhone!=='' && inputAdress!=='' ? actions.editContact(inputName, inputEmail, inputPhone, inputAdress, store.contacts[params.theid].id) : alert('You must complete all fields! Your contact has not been saved.'))} >
						Save
					</button>
				</div>
			</Link>
